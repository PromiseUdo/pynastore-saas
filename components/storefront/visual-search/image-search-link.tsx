 ...
 *
 * One component so the homepage hero, the search page and the mobile search
 * sheet cannot drift into three different labels for the same route (§8,
 * §9). It is a real <Link> to a real page — not a modal, not a launcher —
 * because visual search is one of the store's discovery methods, not a
 * floating assistant bolted onto the corner of the screen.
 *
...
export function ImageSearchLink({
   ...
   *  sheet; `quiet` is an inline text link beside a search field */
  variant?: 'pill' | 'row' | 'quiet';
  label?: string;
  className?: string;
  ...
  return (
    <Link href={VISUAL_SEARCH_PATH} onClick={onNavigate} className={cn(styles, className)}>
